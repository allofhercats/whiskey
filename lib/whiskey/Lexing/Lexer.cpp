...
bool Lexer::isSymbolCharFirst(char32_t value) {
	return (value >= 'a' && value <= 'z') || (value >= 'A' && value <= 'Z') || value >= '\x80' || value == '_';
}

bool Lexer::isSymbolChar(char32_t value) {
	return (value >= 'a' && value <= 'z') || (value >= 'A' && value <= 'Z') || (value >= '0' && value <= '9') || value >= '\x80' || value == '_';
}
...
Lexer::Lexer(Location start, std::vector<Token> &tokens, MessageBuffer &msgs) : ctx(start, tokens, msgs) {}
	...
	while (ctx.areMoreChars()) {
		if (isWhitespaceChar(ctx.getChar())) {
			...
			if (ctx.getChar() == '=') {
				...
				ctx.getMsgs().describe() << "! can only be used in !=, use keyword not instead";
