...
int Source::getEncodingWidth(Source::Encoding encoding) {
	switch (encoding) {
		case Auto: W_ASSERT_UNREACHABLE("Cannot get width of encoding Auto.");
			break;
		case UTF8: return 1;
...
bool Source::isEncodingBE(Source::Encoding encoding) {
	switch (encoding) {
		case Auto: W_ASSERT_UNREACHABLE("Cannot get endianness of encoding Auto.");
			break;
		case UTF8: W_ASSERT_UNREACHABLE("Cannot get endianness of encoding UTF8.");
			break;
		case UTF16LE: return false;
		case UTF16BE: return true;
		case UTF32LE: return false;
		case UTF32BE: return true;
	}
}
...
bool Source::load(void *data, unsigned int width, size_t length, bool copy) {
	...
	W_ASSERT_TRUE(width == 1 || width == 2 || width == 4, "Unsupported character width " << width << ".");
...
Source::Source(std::string path) : path(path), owned(false), data(nullptr), width(0), length(0) {
...
bool Source::loadFile(Source::Encoding encoding) {
	...
	W_ASSERT_LE(bomCount, l, "Could not read more than " << bomCount << " byte" << (bomCount == 1 ? "" : "s") << " of a " << l << " byte file.");

	if (bom[0] == '\xef' && (bom[1] == '\xbb' && bom[2] == '\xbf')) {
		W_ASSERT_TRUE(encoding == Auto || encoding == UTF8, "Detected encoding conflicts with specified.");
		encoding = UTF8;
	} else if (bom[0] == '\xff' && (bom[1] == '\xfe' && (bom[2] == '\x00' && bom[3] == '\x00'))) {
		W_ASSERT_TRUE(encoding == Auto || encoding == UTF32LE, "Detected encoding conflicts with specified.");
		encoding = UTF32LE;
	} else if (bom[0] == '\x00' && (bom[1] == '\x00' && (bom[2] == '\xfe' && bom[3] == '\xff'))) {
		W_ASSERT_TRUE(encoding == Auto || encoding == UTF32BE, "Detected encoding conflicts with specified.");
		encoding = UTF32BE;
	} else if (bom[0] == '\xff' && bom[1] == '\xfe') {
		W_ASSERT_TRUE(encoding == Auto || encoding == UTF16LE, "Detected encoding conflicts with specified.");
		encoding = UTF16LE;
	} else if (bom[0] == '\xfe' && bom[1] == '\xff') {
		W_ASSERT_TRUE(encoding == Auto || encoding == UTF16BE, "Detected encoding conflicts with specified.");
		encoding = UTF16BE;
	} else {
		W_ASSERT_TRUE(encoding == Auto || encoding == UTF8, "Detected encoding conflicts with specified.");
		encoding = UTF8;
