...
namespace whiskey {
String::Length StringContainer::getLowerBoundPow2(String::Length value, String::Length min) {
...
StringContainer::StringContainer(const char *value, StringContainer::Length length) : String() {
	width = 1;
...
StringContainer::StringContainer(const char16_t *value, StringContainer::Length length) : String() {
	width = 2;
	this->length = length == 0 ? getLength(value) : length;
	capacity = getLowerBoundPow2(length+1);
...
StringContainer::StringContainer(const char32_t *value, StringContainer::Length length) : String() {
	width = 4;
	this->length = length == 0 ? getLength(value) : length;
	capacity = getLowerBoundPow2(length+1);
...
StringContainer::StringContainer(const wchar_t *value, StringContainer::Length length) : String() {
	this->length = length == 0 ? getLength(value) : length;
	capacity = getLowerBoundPow2(length+1);
	if (sizeof(wchar_t) == 1) {
		width = 1;
...
StringContainer::StringContainer(const std::string &value) : StringContainer(value.c_str(), value.size()) {}

StringContainer::StringContainer(const std::u16string &value) : StringContainer(value.c_str(), value.size()) {}

StringContainer::StringContainer(const std::u32string &value) : StringContainer(value.c_str(), value.size()) {}

StringContainer::StringContainer(const std::wstring &value) : StringContainer(value.c_str(), value.size()) {}
...
Char8 *StringContainer::getData8() {
	W_ASSERT_EQ(width, 1, "Cannot access char width " << width << " string as 1-byte wide.");
	return data.asChar8;
}

Char16 *StringContainer::getData16() {
	W_ASSERT_EQ(width, 2, "Cannot access char width " << width << " string as 2-byte wide.");
	return data.asChar16;
}

Char32 *StringContainer::getData32() {
	W_ASSERT_EQ(width, 4, "Cannot access char width " << width << " string as 4-byte wide.");
...
void StringContainer::append(const String &value) {
	...
	if (width == value.width) {
		memcpy(data.asChar8 + (length * width), value.data.asChar8, value.length * value.width);
		length += value.length;
	} else {
		Length pos = 0;
