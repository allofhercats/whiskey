...
Char32 readCharUTF16(const Char16 *src, CharOffset &pos, CharOffset length, Endianness endianness) {
	W_ASSERT_NONNULL(src, "Cannot read character from null string.");
	W_ASSERT_LT(pos, length, "Cannot read past end of buffer.");
	W_ASSERT_GE(pos, 0, "Cannot read before start of buffer.");
	W_ASSERT_GE(length, 0, "Cannot read buffer with negative length.");
...
Char32 readCharUTF32(const Char32 *src, CharOffset &pos, CharOffset length, Endianness endianness) {
	W_ASSERT_NONNULL(src, "Cannot read character from null string.");
	W_ASSERT_LT(pos, length, "Cannot read past end of buffer.");
	W_ASSERT_GE(pos, 0, "Cannot read before start of buffer.");
	W_ASSERT_GE(length, 0, "Cannot read buffer with negative length.");
...
void writeCharUTF16(Char16 *dst, Char32 value, CharOffset &pos, CharOffset length, Endianness endianness) {
	W_ASSERT_NONNULL(dst, "Cannot write character from null string.");
	W_ASSERT_LT(pos, length, "Cannot write past end of buffer.");
	W_ASSERT_GE(pos, 0, "Cannot write before start of buffer.");
	W_ASSERT_GE(length, 0, "Cannot write buffer with negative length.");
	...
	if (value <= 0xdf77) {
		...
		W_ASSERT_UNREACHABLE("Cannot convert codepoint in 0xd800 to 0xdfff range to UTF-16.");
	} else if (value <= 0xffff) {
		dst[pos] = value & 0xffff;
		pos++;
	} else if (value <= 0x10ffff) {
		...
		W_ASSERT_UNREACHABLE("Cannot convert code point greater than 0x10ffff to UTF-16.");
	}
}

void writeCharUTF32(Char32 *dst, Char32 value, CharOffset &pos, CharOffset length, Endianness endianness) {
	W_ASSERT_NONNULL(dst, "Cannot write character from null string.");
	W_ASSERT_LT(pos, length, "Cannot write past end of buffer.");
	W_ASSERT_GE(pos, 0, "Cannot write before start of buffer.");
	W_ASSERT_GE(length, 0, "Cannot write buffer with negative length.");
