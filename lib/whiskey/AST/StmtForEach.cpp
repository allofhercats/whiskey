...
void StmtForEach::onGetChildrenStmtForEach(
    std::queue<ContainerRef<AST>> &children) {
}

StmtForEach::StmtForEach(Container<Decl> value,
                         Container<Expr> sequence,
                         Container<Stmt> bodyClause)
    : Stmt(AST::ID::StmtForEach, Range()), value(value), sequence(sequence),
      bodyClause(bodyClause) {
}

StmtForEach::StmtForEach(Range range,
