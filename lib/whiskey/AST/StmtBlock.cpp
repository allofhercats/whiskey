...
void StmtBlock::onGetChildrenStmtBlock(
    std::queue<ContainerRef<AST>> &children) {
}

StmtBlock::StmtBlock(std::vector<Container<Stmt>> body)
    : Stmt(AST::ID::StmtBlock, Range()), body(body) {
}

StmtBlock::StmtBlock(Range range, std::vector<Container<Stmt>> body)
    : Stmt(AST::ID::StmtBlock, range), body(body) {
}
