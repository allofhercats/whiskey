...
StmtFor::StmtFor(std::vector<Container<Decl>> decls,
                 Container<Expr> condition,
                 std::vector<Container<Expr>> steps,
                 Container<Stmt> bodyClause)
    : Stmt(AST::ID::StmtFor, Range()), decls(decls), condition(condition),
      steps(steps), bodyClause(bodyClause) {
}

StmtFor::StmtFor(Range range,
