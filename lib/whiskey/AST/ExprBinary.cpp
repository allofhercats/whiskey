...
void ExprBinary::onGetChildrenExprBinary(
    std::queue<ContainerRef<AST>> &children) {
}

ExprBinary::ExprBinary(AST::ID id, Container<Expr> lhs, Container<Expr> rhs)
    : Expr(id, Range()), lhs(lhs), rhs(rhs) {
}

ExprBinary::ExprBinary(AST::ID id,
                       Range range,
                       Container<Expr> lhs,
                       Container<Expr> rhs)
    : Expr(id, range), lhs(lhs), rhs(rhs) {
