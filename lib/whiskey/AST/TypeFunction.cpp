...
void TypeFunction::onGetChildrenTypeFunction(
    std::queue<ContainerRef<AST>> &children) {
}

TypeFunction::TypeFunction(Container<Type> ret,
                           std::vector<Container<Type>> args)
    : Type(AST::ID::TypeFunction, Range()), ret(ret), args(args) {
}

TypeFunction::TypeFunction(Range range,
                           Container<Type> ret,
                           std::vector<Container<Type>> args)
    : Type(AST::ID::TypeFunction, range), ret(ret), args(args) {
}
