...
#define _W_ASSERT_PRINT_MSG(desc)                                              \
  {                                                                            \
    _W_ASSERT_STREAM << ::whiskey::Color::reset << __FILE__ << ":" << __LINE__ \
                     << ": " << ::whiskey::Color::magentaLight                 \
                     << "assertion failed: " << ::whiskey::Color::white        \
                     << desc << ::whiskey::Color::reset << "\n";               \
  }

#define _W_ASSERT_PRINT_REASON(reason)                           \
  {                                                              \
    _W_ASSERT_STREAM << ::whiskey::Color::greyLight              \
                     << "  reason: " << ::whiskey::Color::reset; \
...
#define _W_ASSERT_DIE()                          \
  {                                              \
    ::whiskey::printBacktrace(_W_ASSERT_STREAM); \
    abort();                                     \
  }
...
#define W_ASSERT_TRUE(arg0, reason)                                        \
  {                                                                        \
    ...
    if (!_w_arg0Value) {                                                   \
      ...
      _W_ASSERT_STREAM << ::whiskey::Color::greyLight                      \
                       << "  actual: " << ::whiskey::Color::reset << #arg0 \
                       << " (false)\n";                                    \
      _W_ASSERT_PRINT_REASON(reason);                                      \
      _W_ASSERT_DIE();                                                     \
    }                                                                      \
...
#define W_ASSERT_LT(arg0, arg1, reason)                                 \
  {                                                                     \
    ...
    if (!(_w_arg0Value < _w_arg1Value)) {                               \
      ...
      _W_ASSERT_STREAM << ::whiskey::Color::greyLight                   \
                       << "  lhs: " << ::whiskey::Color::reset << #arg0 \
                       << " (" << _w_arg0Value << ")\n";                \
      _W_ASSERT_STREAM << ::whiskey::Color::greyLight                   \
                       << "  rhs: " << ::whiskey::Color::reset << #arg1 \
                       << " (" << _w_arg1Value << ")\n";                \
      _W_ASSERT_PRINT_REASON(reason);                                   \
      _W_ASSERT_DIE();                                                  \
    }                                                                   \
