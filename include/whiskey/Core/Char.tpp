namespace whiskey {
template<typename T>
Char32 readCharUTF(const T *src, CharOffset &pos, CharOffset length, Endianness endianness) {
	if (sizeof(T) == 1) {
...
template<typename T>
void writeCharUTF(T *dst, Char32 value, CharOffset &pos, CharOffset length, Endianness endianness) {
	if (sizeof(T) == 1) {
