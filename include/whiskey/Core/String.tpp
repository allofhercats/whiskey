...
template<typename T>
Char32 String::readCharFromString(const T *src, String::Length &pos, String::Length length) {
	...
	if (sizeof(T) == 1) {
		...
		if (src[pos] - 0xd800 < 2048) {
			...
			W_ASSERT_TRUE((src[pos] & 0xfffffc00) == 0xd800, "First 2-byte char in 4-byte codepoint must be high surrogate.");
			Char32 rtn = src[pos] << 10;
			pos++;
			W_ASSERT_LT(pos, length, "High surrogate 2-byte char expects low surrogate, not end of string.");
			W_ASSERT_TRUE((src[pos] & 0xfffffc00) == 0xdc00, "First 2-byte char in 4-byte codepoint must be high surrogate.");
...
template<typename T>
void String::writeCharToString(Char32 chr, T *dst, String::Length &pos, String::Length length) {
